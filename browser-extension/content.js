...
class OpalAuctionDetector {
  constructor() {
    this.trackerUrl = 'https://your-replit-app.replit.app'; // Will be configurable
  ...
  extractAuctionDetails(text, postId) {
    try {
      ...
      // Extract end time (if mentioned)
      const timeMatch = text.match(/(?:ends?|closing|finish).*?(\d{1,2}:\d{2}|\d{1,2}\s*(?:am|pm))/i);
      let endTime = '';
      if (timeMatch) {
  ...
  extractBidsFromText(text) {
    ...
    while ((match = this.bidPattern.exec(text)) !== null) {
      const amount = parseFloat(match[1] || match[2] || match[3]);
